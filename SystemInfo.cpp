...
double SystemInfo::getCPUUsage() {
    ...
    unsigned long long dT = total - prevTotalTicks;
    unsigned long long dI = idle - prevIdleTicks;
    prevTotalTicks = total;
    prevIdleTicks = idle;

    if (dT == 0) return smoothUsage;
    double instant = 100.0 * (1.0 - ((double)dI / (double)dT));
    smoothUsage = (instant * 0.1) + (smoothUsage * 0.9);
    return smoothUsage;
}
